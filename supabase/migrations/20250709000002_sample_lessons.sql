...
-- Mathematics lessons
INSERT INTO public.lessons (title, title_sw, description, description_sw, content, content_sw, subject_id, grade_level, difficulty, estimated_duration, language, cultural_adaptations) 
SELECT * FROM (
    VALUES (
...
) AS lesson_data(title, title_sw, description, description_sw, content, content_sw, subject_id, grade_level, difficulty, estimated_duration, language, cultural_adaptations)
WHERE NOT EXISTS (
    SELECT 1 FROM public.lessons WHERE title = lesson_data.title
);

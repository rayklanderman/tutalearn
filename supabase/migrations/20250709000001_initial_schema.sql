...
INSERT INTO public.subjects (name, name_sw, description, icon) 
SELECT * FROM (
    VALUES 
...
) AS new_subjects(name, name_sw, description, icon)
WHERE NOT EXISTS (
    SELECT 1 FROM public.subjects WHERE name = new_subjects.name
);
...
DROP POLICY IF EXISTS "Users can view own conversations" ON public.whatsapp_conversations;
